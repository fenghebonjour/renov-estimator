...
export class JvmMetricsComponent {
  // Local UI state as a signal — no HTTP here, just a demonstration that signals
  // model plain component state too, not only async data.
  ...
  // computed() derives from activeId(); it re-evaluates only when activeId changes.
  active = computed<Scenario | null>(() => this.scenarios.find(s => s.id === this.activeId()) ?? null);
